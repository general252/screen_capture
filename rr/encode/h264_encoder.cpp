...
int ffmpeg::H264Encoder::Encode(uint8_t* in_buffer, uint32_t in_width, uint32_t in_height, uint32_t image_size, std::vector<uint8_t>& out_frame)
{
    out_frame.clear();

    if (!this->GetAVCodecContext()) {
        return -1;
    }


    int frame_size = 0;
    int max_buffer_size = av_config_.video.width * av_config_.video.height * 4;
...
inline bool ffmpeg::H264Encoder::IsKeyFrame(const uint8_t* data, uint32_t size)
{
    if (size > 4) {
